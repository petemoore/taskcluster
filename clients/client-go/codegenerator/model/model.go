...
import (
	...
	"github.com/taskcluster/taskcluster/v99/tools/jsonschema2go"
...
func (apiDefs APIDefinitions) GenerateCode(goOutputDir string) {
	...
	for i := range apiDefs {
		if strings.Contains(apiDefs[i].PackageName, "events") {
			amqpApiLinks.WriteString("\n" + "* https://pkg.go.dev/github.com/taskcluster/taskcluster/v99/clients/client-go/" + apiDefs[i].PackageName + "\n")
